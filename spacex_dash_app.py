...
def get_pie_chart(entered_site):
    if entered_site == 'ALL':
        ...
        ).to_frame().reset_index().rename(columns={"class": "nb_success"})
        ...
        filtered_df = filtered_df.rename(
            columns={"index": "class", "class": "count"})
...
def get_scatter_chart(entered_site, slider_value):
    if entered_site == 'ALL':
        ...
        filtered_df = filtered_df[filtered_df['Payload Mass (kg)'].between(
            slider_value[0], slider_value[1], inclusive=True)]
        fig = px.scatter(filtered_df, x='Payload Mass (kg)', y='class', color='Booster Version Category',
                         title='Correlation between Payload and Success for all Sites')
        return fig
    else:
        filtered_df = spacex_df[spacex_df['Launch Site'] ==
                                entered_site]['Payload Mass (kg)', 'class', 'Booster Version Category']
        filtered_df = filtered_df[filtered_df['Payload Mass (kg)'].between(
            slider_value[0], slider_value[1], inclusive=True)]
        fig = px.scatter(filtered_df, x='Payload Mass (kg)', y='class', color='Booster Version Category',
